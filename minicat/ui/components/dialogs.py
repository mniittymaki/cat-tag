    ...
    with ui.dialog() as burn_dialog, ui.card().classes("w-[520px]"):
        ...
        async def do_burn():
            try:
                ...
                segs = getattr(v, "transcription_segments", None) if lang == "original" else translations.get(lang)
                if not segs:
                ...
                await asyncio.to_thread(
                    ...
                    v.path, srt_out, final_video,
                    use_ebu_style=True, progress_callback=progress_callback
                )
                ...
                def refresh_logo_preview(path: str | None):
                    ...
                    with logo_preview:
                        if path and Path(path).exists():
                            ui.image(path).classes("w-12 h-12 rounded-full object-cover border border-zinc-700")
                        else:
        ...
        if projects_under_client:
            for p in projects_under_client:
                with ui.row().classes("items-center gap-2 mb-1"):
                    ...
                    ui.button(icon="info", on_click=lambda pp=p: (show_rich_project_dialog(state, pp), dialog.close())).props("size=sm flat dense round")
        else:
        ...
        def do_delete():
            ...
            if also_delete:
                ...
                try:
                    ...
                    clips_in_project = [v for v in getattr(state, "videos", []) if getattr(v, "project", None) == current_name]
                    for v in clips_in_project:
                        if getattr(v, "id", None):
                            try:
                                cleanup_all_generated_files_for_clip(v.id, state.catalog_root, original_filename=getattr(v, "filename", None))
            ...
            affected = _db.delete_project(state.catalog_root, current_name, also_delete_clips=also_delete)
            dialog.close()
            state.clear_filters()
...
def show_rich_project_dialog(state: Any, name: str):
    ...
    from minicat.core import db as _db
    from minicat.core.models import Client, SearchFilters
    ...
    with ui.dialog() as dialog, ui.card().classes("w-[620px]"):
        ...
        # === Project Metadata ===
        with ui.column().classes("gap-2"):
            start = ui.input("Start Date", value=str(proj.start_date) if proj.start_date else "").props("dense")
            end = ui.input("End Date", value=str(proj.end_date) if proj.end_date else "").props("dense")
        ...
        ui.label("Leave fields empty to keep existing values. You can add tags to every clip here.").classes("text-xs text-grey-6 mb-2")

        with ui.column().classes("gap-2"):
            batch_date = ui.input(
                "Date", 
                value=str(template.shoot_date) if template and template.shoot_date else ""
            ).props("dense")

            batch_location = ui.input(
                "Location", 
                value=template.location or "" if template else ""
            ).props("dense")

            batch_operator = ui.input(
                "Operator", 
                value=template.operator or "" if template else ""
            ).props("dense")
        ...
        def apply_batch():
            ...
            new_tags = [t.strip() for t in batch_tags.value.split(",") if t.strip()] if batch_tags.value else []
        ...
        with ui.row().classes("justify-between gap-2 mt-4 w-full"):
            ui.button("Apply to all clips in Project", icon="check", on_click=apply_batch, color="primary")
...
def delete_client_dialog(state: Any, client: Any):
    ...
    with ui.dialog() as dialog, ui.card().classes("w-96"):
        ...
        ui.label("This will permanently delete the client record from the database and remove it from all associated projects. Projects themselves will not be deleted.").classes("text-xs text-grey-6 mb-3")
...
def show_storyboard_dialog(video: Any) -> None:
    ...
    with ui.dialog() as dialog, ui.card().classes("w-[92vw] max-w-[1200px] q-pa-none overflow-hidden"):
        ...
        big_img = ui.image(str(video.storyboard_path)).classes("w-full cursor-pointer").style("max-height: 70vh; object-fit: contain;")
        ...
        with ui.row().classes("px-4 py-2 bg-[#111] justify-between"):
            ...
            ui.button("Ask AI for tags", icon="auto_awesome", on_click=ask_ai).props("size=sm outline")

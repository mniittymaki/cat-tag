    ...
    with ui.element("div").classes("flex flex-wrap gap-1 q-px-0"):
        ...
        for tag, count in most_used:
            ...
            badge_classes = (
                ...
                if is_active
                else "rounded px-1.5 py-0.5 text-xs cursor-pointer bg-zinc-800 text-zinc-300 border-zinc-700 hover:bg-zinc-700"
            )
...
def left_drawer_content(
    ...
    with ui.column().classes("w-full p-0 m-0 gap-0 select-none"):
        # === BROWSE ===
        with (
            ui.expansion("BROWSE", icon="home", value=True)
            .classes(
                "w-full text-xs font-semibold text-zinc-400 tracking-wider q-py-none min-h-[36px]"
            )
            .props('header-class="text-zinc-400" dense disable-toggle')
        ):
            ...
            with (
                ui.row()
                .classes(
                    "w-full items-center justify-between hover:bg-zinc-800 rounded q-px-xs py-0.5 cursor-pointer text-sm"
                )
                .on("click", show_all_media)
            ):
                ...
                ui.badge(str(len(state.videos)), color="zinc-700").classes(
                    "text-xs text-zinc-400 q-px-1"
                )
        ...
        # === CLIENTS ===
        with (
            ui.expansion("CLIENTS", icon="business", value=True)
            .classes(
                "w-full text-xs font-semibold text-zinc-400 tracking-wider q-py-none min-h-[36px]"
            )
            .props('header-class="text-zinc-400" dense disable-toggle')
            ...
            ui.button("NEW CLIENT", icon="add", on_click=create_new_client).props(
                "size=sm flat dense color=primary"
            ).classes("q-px-xs text-xs font-medium self-start")
            ...
            for v in state.videos:
                p = v.project or "(none)"
                proj_clients = (
                    db.get_clients_for_project(state.catalog_root, p) if p != "(none)" else []
                )
                if not proj_clients:
            ...
            for client_name in list(state.all_clients):
                client_obj = next(
                    (c for c in db.get_clients(state.catalog_root) if c.name == client_name), None
                )
                if not client_obj:
                    continue
            ...
            for client_name in sorted(client_structure.keys()):
                ...
                is_active_client = bool(
                    state.filters.client and client_name in state.filters.client
                )
                ...
                with ui.row().classes(client_row_classes):
                    ...
                    with ui.row().classes("w-full items-center justify-between group"):
                        with (
                            ui.row()
                            .classes("items-center flex-1 gap-x-1 no-wrap")
                            .on(
                                "click",
                                lambda cn=client_name: (state.toggle_client(cn), ui.update()),
                            )
                        ):
                            ...
                            ui.badge(str(total_for_client), color="zinc-700").classes(
                                "text-xs text-zinc-400 q-px-1 shrink-0"
                            )

                        # Subtle direct Edit icon + three-dot menu
                        with ui.row().classes(
                            "items-center opacity-0 group-hover:opacity-100 transition-opacity shrink-0 no-wrap gap-1"
                        ):
                            client_obj = next(
                                (
                                    c
                                    for c in db.get_clients(state.catalog_root)
                            ...
                            if client_obj:
                                ui.button(
                                    icon="edit",
                                    on_click=lambda cid=client_obj.id: (
                                        ui_dialogs.show_rich_client_dialog(state, cid)
                                    ),
                                ).props("flat dense round size=xs").classes(
                                    "text-zinc-500 hover:text-zinc-300"
                                )
                                with ui.button(icon="more_vert").props(
                                    "size=xs flat dense round text-zinc-600"
                                ):
                                    with ui.menu():
                                        ui.item(
                                            "Edit Client",
                                        ...
                                        ui.item(
                                            "Delete Client",
                                            on_click=lambda c=client_obj: (
                                                ui_dialogs.delete_client_dialog(state, c)
                                            ),
                                        ).props("clickable")
                ...
                for proj_name, proj_count in sorted(projects_dict.items()):
                    ...
                    with ui.row().classes(
                        "group w-full items-center justify-between hover:bg-zinc-800 rounded q-px-xs py-0.5 cursor-pointer text-sm q-pl-md no-wrap"
                    ):
                        ...
                        with ui.row().classes("w-full items-center justify-between group"):
                            with (
                                ...
                                .on(
                                    "click",
                                    lambda p=proj_name: (state.toggle_project(p), ui.update()),
                                )
                            ):
                                ui.icon("folder_open", size="xs").classes("text-zinc-500 shrink-0")
                                ui.label(project_display_name).classes(
                                    "text-sm text-zinc-300 truncate"
                                )
                                ui.badge(str(proj_count), color="zinc-700").classes(
                                    "text-xs text-zinc-400 q-px-1 shrink-0"
                                )
                            ...
                            with ui.row().classes(
                                ...
                                ui.button(
                                    icon="edit",
                                    on_click=lambda p=proj_name: (
                                        ui_dialogs.show_rich_project_dialog(state, p)
                                    ),
                                ).props("flat dense round size=xs").classes(
                                    "text-zinc-500 hover:text-zinc-300"
                                )
                                with ui.button(icon="more_vert").props(
                                    ...
                                    with ui.menu():
                                        ...
                                        ui.item(
                                            "Delete Project",
                                            on_click=lambda p=proj_name: (
                                                ui_dialogs.delete_project_dialog(state, p)
                                            ),
                                        ).props("clickable")
        ...
        with (
            ui.expansion("LOCATIONS", icon="place", value=False)
            .classes(
                "w-full text-xs font-semibold text-zinc-400 tracking-wider q-py-none min-h-[36px]"
            )
            .props('header-class="text-zinc-400" dense')
        ):
            for loc in state.all_locations[:12]:
                ...
                with (
                    ui.row()
                    .classes(
                        "w-full items-center justify-between hover:bg-zinc-800 rounded q-px-xs py-0.5 cursor-pointer text-sm no-wrap"
                    )
                    .on(
                    ...
                    ui.badge(str(count), color="zinc-700").classes(
                        "text-xs text-zinc-400 q-px-1 shrink-0"
                    )

        # === CAMERAS (collapsible) ===
        with (
            ui.expansion("CAMERAS", icon="videocam", value=False)
            .classes(
                "w-full text-xs font-semibold text-zinc-400 tracking-wider q-py-none min-h-[36px]"
            )
            .props('header-class="text-zinc-400" dense')
            ...
            for cam in state.all_cameras[:15]:
                ...
                with (
                    ...
                    .on("click", lambda c=cam: (state.toggle_camera(c), ui.update()))
                ):
                    with ui.row().classes("items-center gap-x-1 no-wrap flex-1"):
        ...
        with (
            ui.expansion("RICH TAGS", icon="tag", value=False)
            .classes(
                "w-full text-xs font-semibold text-zinc-400 tracking-wider q-py-none min-h-[36px]"
            )
            .props('header-class="text-zinc-400" dense')
...
def create_left_drawer(main_layout_state: Any) -> ui.left_drawer:
    left_drawer = ui.left_drawer(value=True, fixed=True, elevated=False).props(
        "width=300 behavior=desktop bordered"
    )
    left_drawer.classes(
        "bg-[#111112] text-zinc-300 border-r border-zinc-800/40 q-pa-none overflow-hidden"
    )
